<template>
  <v-container>
    ...
    <v-row>
      ...
      <v-col cols="3">
        <v-combobox v-model="filtered_type" :items="unique_type" label="選擇型式" multiple chips></v-combobox>
      </v-col>
      <v-col cols="3">
        <v-autocomplete
          v-model="filtered_website"
          :items="unique_website"
          :search-input.sync="search_website"
          :multiple="true"
          :return-object="true"
          :chips="true"
      ...
      <v-col cols="3">
        <v-combobox
          v-model="filtered_industry"
          :items="unique_industry"
          label="選擇產業分類"
          :multiple="false"
          :height="42"
        ></v-combobox>
      ...
      <v-col cols="1">
        <v-checkbox v-model="filterStatusNo" label="去除 no 頻道"></v-checkbox>
      </v-col>
    </v-row>
    <v-row>
      <v-col>
    ...
    <v-row>
      <v-col>
        <div class="text-center pb-2">
          <v-btn
            color="normal"
            @click="showShortSelectedChannels=!showShortSelectedChannels"
          >{{ showShortSelectedChannels? "關閉":"顯示"}}已選頻道</v-btn>
        </div>
        <v-card class="mx-auto" outlined v-if="showShortSelectedChannels">
          <v-card-text>
            <v-chip
              class="ma-2"
              close
              @click:close="removeChannel($event, channel.tkfc_name)"
              v-for="(channel, index) in selected"
              :key="index"
              v-bind:class="{'pink lighten-3': channel.tw_status === false, 'indigo lighten-3': channel.tw_status === true}"
            >{{channel.display_name}}</v-chip>
          </v-card-text>
        </v-card>
      </v-col>
    </v-row>
    ...
    <v-row>
      <v-col>
        <div>
          <a
            href="http://api.i-buzz-system.com/load-data/downloads/inner-tool/bangyan-sample.xlsx"
          >PRO 範例檔案</a>
        </div>
      </v-col>
    </v-row>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "頻道中文名", value: "display_name" },
      { text: "網站", value: "website", filterable: false },
      //   { text: "url", value: "url" },
      { text: "型式", value: "tw_type" },
      // { text: "圖檔路徑", value: "img_path", filterable: false },
      { text: "狀態", value: "tw_status", filterable: false },
      { text: "備註", value: "note", filterable: false }
